import type { SpotDTO } from '@shared/types';

export interface SpotWithCams extends SpotDTO {
  cameras: { label: string; embedUrl: string }[];
}

export const DEMO_SPOTS: SpotWithCams[] = [
  {
    id: 'tel-aviv-hilton',
    name: 'Hilton Beach',
    lat: 32.0907, lng: 34.7707,
    country: 'Israel', region: 'Tel Aviv',
    cameras: [
      { label: 'Tel Aviv Port Cam', embedUrl: 'https://www.youtube.com/embed/live_stream?channel=UCKRJfM-PmFcnHEXGhVjxKjA' },
    ],
  },
  {
    id: 'herzliya',
    name: 'Herzliya',
    lat: 32.1640, lng: 34.7920,
    country: 'Israel', region: 'Sharon',
    cameras: [],
  },
  {
    id: 'ashdod',
    name: 'Ashdod',
    lat: 31.7920, lng: 34.6360,
    country: 'Israel', region: 'Southern District',
    cameras: [],
  },
  {
    id: 'haifa-bat-galim',
    name: 'Bat Galim',
    lat: 32.8260, lng: 34.9610,
    country: 'Israel', region: 'Haifa',
    cameras: [],
  },
  {
    id: 'netanya',
    name: 'Netanya',
    lat: 32.3210, lng: 34.8530,
    country: 'Israel', region: 'Sharon',
    cameras: [],
  },
  {
    id: 'pipeline',
    name: 'Pipeline',
    lat: 21.6650, lng: -158.0530,
    country: 'USA', region: 'Hawaii',
    cameras: [
      { label: 'Pipe Masters Cam', embedUrl: 'https://www.youtube.com/embed/live_stream?channel=UCH1LrJKHEA0CxdQjHJyL2lA' },
    ],
  },
  {
    id: 'jeffreys-bay',
    name: "Jeffrey's Bay",
    lat: -34.0488, lng: 24.9117,
    country: 'South Africa', region: 'Eastern Cape',
    cameras: [
      { label: 'J-Bay Surf Cam', embedUrl: 'https://www.youtube.com/embed/live_stream?channel=UCJays_bay_surf' },
    ],
  },
  {
    id: 'nazare',
    name: 'Nazaré',
    lat: 39.6010, lng: -9.0700,
    country: 'Portugal', region: 'Centro',
    cameras: [
      { label: 'Nazaré Canyon Cam', embedUrl: 'https://www.youtube.com/embed/live_stream?channel=UCNazare_surf' },
    ],
  },
  {
    id: 'mavericks',
    name: 'Mavericks',
    lat: 37.4920, lng: -122.5010,
    country: 'USA', region: 'California',
    cameras: [
      { label: 'Mavericks Surf Cam', embedUrl: 'https://www.youtube.com/embed/live_stream?channel=UCMavericks_surf' },
    ],
  },
  {
    id: 'biarritz',
    name: 'Biarritz',
    lat: 43.4830, lng: -1.5586,
    country: 'France', region: 'Nouvelle-Aquitaine',
    cameras: [
      { label: 'Grande Plage Cam', embedUrl: 'https://www.youtube.com/embed/live_stream?channel=UCBiarritz_surf' },
    ],
  },
];
